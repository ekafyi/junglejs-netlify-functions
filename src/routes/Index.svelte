...
<main class="p-4 mx-auto max-w-3xl lg:max-w-5xl">
  <h1 class="text-green-500 font-bold mb-6 text-4xl md:text-6xl">
    My Reading List
  </h1>
    ...
    {#each QUERYRES.books as { authors, cover, subtitle, title, url }}
      <article class="p-2 sm:p-6">
        <img
          ...
          width="180"
          height="270"
          class="book-cover rounded-lg shadow-md w-full object-cover" />
        <div class="mt-4">
          <h3 class="font-bold text-md mb-1 leading-tight">
            {`${title}${subtitle ? `: ${subtitle}` : ''}`}
          </h3>
          <span class="text-gray-600 text-sm">{joinAuthorNames(authors)}</span>
        </div>
      </article>
    {:else}
